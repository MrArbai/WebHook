        ...
        // $file_id = 
            if ($perintahs[0]=='STBPB') {
                ...
                if ($message_data["photo"] == NULL) {
                    ...
                    $bot->sendMessage($this->chat_id, "Serah Terima Sukses!" , 'HTML', NULL, NULL, NULL);
...
function isOneloginExists($TeleID) {
    $bot = new TelegramBot\Api\BotApi($this->token);
    $sql="SELECT PersonalID FROM telebot..tbl_OL_Onelogin where TelegramID = '" . $this->chat_id ."' ";
    ...
    if ($user == NULL) {
       ...
       return 0;
        ...
        $this->UserIDOneLogin = $user['PersonalID'] ;
        return 1;
    }
...
function isUserAppExists() {
    ...
    if ($this->isOneloginExists($this->chat_id) == 1) {
        ...
        $sql = "SELECT A.TelegramID,A.PersonalID,B.UserID ,D.LoginID,D.LoginName,D.PositionName,D.InActive,D.GrpID 
                FROM telebot..tbl_OL_Onelogin A
                LEFT JOIN OneLogin..tbl_OL_MstUser B ON A.PersonalID = B.UserID
                LEFT JOIN OneLogin..tbl_OL_MstUserProgram C ON B.UserID = C.UserID
                LEFT JOIN MYPSG..vwuser D ON C.UserID1 = D.LoginID
                where C.ProgramID='MyPSG' 
                AND A.TelegramID = '" . $this->chat_id . "'" ;
